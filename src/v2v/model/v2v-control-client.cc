...
TypeId V2vControlClient::GetTypeId(void) {
	static TypeId tid =
            ...
            .AddTraceSource ("Status", "Status chenged",
                      MakeTraceSourceAccessor (&V2vControlClient::m_statusTrace),
                    "ns3::V2V2vControlClient::StatusTraceCallback");
	return tid;
}
...
const V2vClusterSap::NeighborInfo
V2vControlClient::GetCurrentMobility() const {
  const V2vClusterSap::NeighborInfo info = m_currentMobility;
  return info;
}
...
V2vControlClient::HandleRead (Ptr<Socket> socket) {
    ...
    V2vClusterSap::NeighborInfo prev_mobility = m_currentMobility;
...
V2vControlClient::Send (void) {
  ...
  // if(m_sendEvent.IsExpired()) return; // for debug
